...
def run_simulation(
    ...
    for sim_month in range(1, months + 1):
        ...
        # Hospital costs: fixed base ($101M/yr) + homeless-attributable bed-days
...
def print_summary(scenarios: dict):
    ...
    print("  (Hospital base = $101M/yr fixed; PEH = homeless-attributable bed-day costs)")
    print("=" * w)
...
def plot_cost_breakdown(scenarios: dict, months: int = SIM_MONTHS):
    ...
    Stacked bar with THREE layers per scenario:
      1. Hospital base operating cost (grey)  — fixed $101M/yr × sim years
    ...
    bars_b = ax.bar(x, base_cum, w,
                    label="Hospital base operating cost ($101M/yr fixed)",
                    color="#95a5a6", alpha=0.88)
    bars_p = ax.bar(x, peh_cum, w, bottom=base_cum,
                    label="PEH-attributable bed-day costs (varies by scenario)",
                    color="#e74c3c", alpha=0.88)
    bars_s = ax.bar(x, shlt_cum, w,
    ...
    ax.set_title(
        ...
        f"Hospital base operating cost ($101M/yr) included for full financial context",
        fontsize=11
    )
